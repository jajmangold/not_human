...
# Serializes the GPU forward of the (shared) BiSeNet across prepare threads — concurrent forward on one
# CUDA module triggers an illegal memory access. CPU post-processing runs outside this lock.
_FP_GPU_LOCK = threading.Lock()
...
class FaceParsing():
    ...
    def __call__(self, image, size=(512, 512), mode="raw"):
        ...
        with torch.no_grad():
            ...
            # Serialize ONLY the GPU forward+transfer across threads (concurrent net() calls cause a
            # CUDA illegal-memory-access). The net is ~9ms; the heavy work (argmax over [19,512,512] +
            # cv2 morphology, ~300ms) stays OUTSIDE the lock so it parallelizes across the prepare threads.
            with _FP_GPU_LOCK:
                if torch.cuda.is_available():
                ...
                out = self.net(img)[0]
                # argmax on GPU (cheap) instead of CPU over [19,512,512]; also shrinks the transfer 19x
